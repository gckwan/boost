...
function drawTypeGraph(condition, type, index, len) {
  ...
  var node = graphSvg.selectAll(".suppNode" + index)
    .data(nodes)
    .enter().append("a").attr("href", function(d) { return d.supplement[8]; })
    .append("g")
  ...
  node.append("circle")
    ...
    .attr("r", function(d, i) { console.log(d); return 30 + (d["supplement"][1] * 10)})
    .style("fill", typeColors[type])
    .style("opacity", function(d) { 
      var otw = d.supplement[5];
      if (otw === "OTW") return 0.7;
      return 1;
    });
...
function drawEvidenceScale() {
  var evidenceSvg = d3.select(".legend").append("svg").attr("class", "evidenceSvg");
  evidenceSvg.selectAll(".evidenceCircle")
    .data([0, 1, 2, 3, 4, 5])
    .enter().append("circle")
    .attr("cx", function(d) { return d * 19 + 19; })
    .attr("cy", 35)
    .style("fill", "#6e88b3")
    .attr("class", "evidenceCircle")
    .attr("r", function(d) { return 10 + d; });

  evidenceSvg.selectAll(".otw")
    .data([0])
    .enter().append("circle")
